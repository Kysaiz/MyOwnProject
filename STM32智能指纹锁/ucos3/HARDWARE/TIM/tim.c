...
volatile uint32_t g_dht11_time = 6;	// dht11读取时间
volatile uint32_t g_lock_time = 15;	// 锁定时间
volatile uint32_t g_key_event = 0;	// 键盘输入事件标志位，0表示未输入，1表示正在输入
volatile uint32_t g_key_time = 0;	// 键盘输入时间，10秒未输入则归零
...
void tim3_init(void)
{
	...
	TIM_TimeBaseStructure.TIM_Period = 10000-1;//计数值5000-1，决定了定时时间500ms
